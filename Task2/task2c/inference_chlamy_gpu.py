"""
Task 2c — TopCUP inference on a Chlamydomonas in-situ chunk + metrics (GPU-only).

Stripped-down variant of inference_chlamy.py for environments where an NVIDIA GPU is
guaranteed (e.g. a cluster). It removes the entire CPU-compatibility layer
(Lightning->CPU patching, .cuda()/.to('cuda') no-ops, CUDA-autocast disabling) the
portable inference_chlamy.py carries. What remains:
  ...
  - chunk extraction, copick config, and metrics (unchanged)

No subprocess wrapper: with the CPU layer gone there is nothing to patch in a clean
interpreter, so TopCUP's CLI is called in-process via cli(standalone_mode=False).
This assumes Linux-style `fork` DataLoader workers (the cluster default); they share
memory with the parent and do NOT re-import this module.

Chunk (task spec): x_offset=160, y_offset=752, size=256 in x and y, full z.
Contains 56 of the 202 ground-truth ribosomes.

This is the held-out, in-situ test. With the same correct 6-class config that scores
F1≈0.76 on synthetic data, the phantom-trained model produces 0 confident ribosome
detections here — the in-distribution → in-situ generalization gap.
"""
...
def make_copick_config(chunk_zarr):
    ...
    if not link.exists() and not link.is_symlink():
        link.symlink_to(chunk_zarr.resolve())
        print(f"  symlink: denoised.zarr → {chunk_zarr.resolve()}")
...
def run_inference(cfg, out_dir):
    ...
    print("  Running TopCUP on chunk (GPU)...")
    try:
...
if __name__ == "__main__":
    ...
    assert torch.cuda.is_available(), \
        "No CUDA GPU found — inference_chlamy_gpu.py requires a GPU. Use inference_chlamy.py for CPU."
    ...
    print(f"\n5. Metrics  (radius = {RADIUS:.0f} vox = 150 Å = 1 ribosome radius)")
