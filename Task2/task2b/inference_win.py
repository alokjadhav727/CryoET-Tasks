...
WINDOWS variant of inference.py. The ONLY difference vs. the original is the
directory-link step:

  Original (Linux/macOS):
      link.symlink_to(actual)            # needs admin/Dev-Mode on Windows

  This file (Windows):
      _make_dir_link(actual, link)       # uses mklink /J (directory junction)
                                         # which works without admin rights

Everything else (copick config, the TopCUP CPU/GPU wrapper, metrics) is byte-
identical to inference.py.
"""
...
# ---------------------------------------------------------------------------
# Everything below this line is the original inference.py logic, unchanged
# except `link.symlink_to(actual)` -> `_make_dir_link(actual, link)`.
# ---------------------------------------------------------------------------
...
def make_copick_config():
    ...
    if not link.exists() and not link.is_symlink():
        _make_dir_link(actual, link)                       # <-- only change
        print(f"  link: denoised.zarr -> {actual}")
...
if __name__ == "__main__":
    ...
    print("\n4. Metrics at standard radius (15 vox = 150 A = 1 ribosome radius):")
